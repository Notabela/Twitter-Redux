...
open class TwitterProfileViewController: UIViewController {
  ...
  open var username: String? {
    ...
    open var screenName: String? {
        didSet {
            self.profileHeaderView?.usernameLabel?.text = screenName
        }
    }
    
  open var totalTweets: Int? {
    didSet {
        self.navigationDetailLabel.text = "\(totalTweets!) Tweets"
      }
    }
  ...
  open var locationString: String? {
   ...
   open var followersCount: String? {
      didSet{
        self.profileHeaderView?.followersCountLabel?.text = followersCount
      }
    }
    
    open var followingCount: String? {
        didSet{
           self.profileHeaderView?.followingCountLabel.text = followingCount
      }
    }
  ...
  open var coverImage: UIImage? {
    didSet {
        ...
        self.headerCoverView.alpha = 0
        self.headerCoverView?.image = coverImage
        
        UIView.animate(withDuration: 0.3) { 
            self.headerCoverView.alpha = 1
        }
    }
  }
...
extension TwitterProfileViewController {
  ...
  func prepareViews() {
    ...
    if let _profileHeaderView = Bundle.bundleFromPod()?.loadNibNamed("TwitterProfileHeaderView", owner: self, options: nil)?.first as? TwitterProfileHeaderView {
      ...
      self.profileHeaderView.titleLabel.text = self.username
      self.profileHeaderView.usernameLabel.text = self.screenName
